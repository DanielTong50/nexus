    ...
    <section id="how-it-works" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          {/* Connecting Line */}
          <div className="hidden lg:block absolute top-1/2 left-0 right-0 h-px bg-slate-200 transform -translate-y-1/2" />
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {STEPS.map((step, index) => (
              ...
              >
                {/* Step Card */}
                <div className="bg-white rounded-2xl p-6 border border-slate-200 hover:border-slate-300 hover:shadow-xl transition-all duration-300 relative z-10">
                  {/* Number Badge */}
                  <div className="w-12 h-12 rounded-full bg-slate-900 flex items-center justify-center text-white font-bold text-lg mb-4">
                    {step.number}
                  </div>
                ...
                {/* Connector Dot */}
                <div className="hidden lg:block absolute top-1/2 left-1/2 w-4 h-4 rounded-full bg-white border-4 border-slate-300 transform -translate-x-1/2 -translate-y-1/2 z-20" />
              </motion.div>
            ))}
          </div>
        </div>
